...
class OVSegDEMO(MaskFormer):
    ...
    def forward(self, batched_inputs):
        ...
        # [12, 768] 12个类别的文本向量
        text_features = self.clip_adapter.get_text_features(class_names)
        # outputs["pred_logits"] [1, 100, 768] 100个proposal的vision向量
        ori_mask_former_embeds = self.clip_adapter.normalize_feature(outputs["pred_logits"])
        # [1, 100, 12] 100个proposal的类别概率
        outputs["pred_logits"] = self.clip_adapter.get_sim_logits(
            text_features, self.clip_adapter.normalize_feature(outputs["pred_logits"])
        )
        mask_cls_results = outputs["pred_logits"]
        ...
        processed_results = []
        for mask_cls_result, mask_pred_result, input_per_image, image_size, ori_mask_former_embed in zip(
            mask_cls_results, mask_pred_results, batched_inputs, images.image_sizes, ori_mask_former_embeds
        ):
            height = image_size[0]
            width = image_size[1]
            mask_pred_result = sem_seg_postprocess(
            ...
            r, regions, cls_index_map, ori_clip_adapter_embed, valid_flag = self.demo_inference(mask_cls_result, mask_pred_result, image, class_names)
            #pdb.set_trace()
            height = input_per_image.get("height", image_size[0])
            width = input_per_image.get("width", image_size[1])
            r = sem_seg_postprocess(r, image_size, height, width)
            processed_results.append({"sem_seg": r, "cls_index_map": cls_index_map, "ori_mask_former_embed": ori_mask_former_embed[valid_flag], "ori_clip_adapter_embed" : ori_clip_adapter_embed, "text_features": text_features})
    ...
    def demo_inference(self, mask_cls, mask_pred, image, class_names):
        ...
        regions = None
        ori_clip_adapter_embed = None
        if self.clip_ensemble:
            # 'CLIP_ADAPTER': CfgNode({'TEXT_TEMPLATES': 'vild', 'PREDEFINED_PROMPT_TEMPLATES': ['a photo of a {}.'],
            # 'PROMPT_CHECKPOINT': '', 'CLIP_MODEL_NAME': 'ViT-L/14', 'MASK_FILL': 'mean', 'MASK_EXPAND_RATIO': 1.0,
            # 'MASK_THR': 0.35, 'MASK_MATTING': False, 'REGION_RESIZED': True, 'CLIP_ENSEMBLE': True, 'CLIP_ENSEMBLE_WEIGHT': 0.0,
            # 'MASK_PROMPT_DEPTH': 3, 'MASK_PROMPT_FWD': True})})
            clip_cls, regions, valid_flag, ori_clip_adapter_embed = self.clip_adapter(
                image, class_names, mask_pred, normalize=True
            )
            if clip_cls is None:
                clip_cls = torch.empty(0, mask_cls.shape[-1] + 1, device=self.device)
            ...
            if self.clip_ensemble_weight > 0:
                ...
                # only clip model predictions are used
                # map_back_clip_cls = mask_cls.new_zeros(mask_cls.shape)
                # map_back_clip_cls[valid_flag] = clip_cls
                # mask_cls = map_back_clip_cls
                mask_cls = clip_cls
                mask_pred = mask_pred[valid_flag]
        ...
        return semseg, regions, cls_index_map, ori_clip_adapter_embed, valid_flag
